 ...
 *   public/data/world-50m.json        Natural Earth Admin-0 boundaries, simplified for drawing
 *   src/data/countries.generated.ts   typed metadata for the ~195 playable countries
 *
 * Note the split: every number the game reasons about — centroids, areas, which
 * countries count as micro-states — is derived from the FULL-resolution geometry, and
 * only the copy that gets drawn is simplified. See the simplification block near the
 * end for why that ordering matters.
 *
...
/**
 * The rendered topology is simplified; the game's maths is not.
 *
 * At full 50m resolution the map is ~101,000 vertices across 244 SVG paths, and every
 * one of them is re-rasterised on each frame of a pan or a pinch — which is what makes
 * the map feel heavy on a phone. Simplification is a straight multiplier on that cost.
 *
 * It is safe to do here specifically because it happens AFTER every centroid, area and
 * micro-state decision above has been computed from the full-resolution geometry. The
 * simplified topology is only ever drawn; nothing the game reasons about is derived
 * from it. Small countries are protected twice over: RETAIN_POINTS is chosen to keep
 * every playable country's ring intact, the result is verified below, and the 39
 * smallest are clickable through markers regardless of how their outline renders.
 */
const RETAIN_POINTS = 0.38;
...
const rawPoints = countPoints(topo50);
const presimplified = presimplify(topo50);
const weight = quantile(presimplified, RETAIN_POINTS);
const thinned = simplify(presimplified, weight);

/*
 * Re-quantise before writing.
 *
 * presimplify hangs a weight on every coordinate and simplify hands back absolute
 * floating-point positions, so the naive output is *larger* than the input it halved
 * the vertex count of — 1.3 MB against 739 KB. Dropping the weights and re-encoding to
 * integer deltas restores the compact form. 1e5 keeps roughly 0.4 km of precision at
 * the equator, far finer than a 50m-resolution source can justify.
 */
for (const arc of thinned.arcs) {
  for (let i = 0; i < arc.length; i++) arc[i] = [arc[i][0], arc[i][1]];
}
delete thinned.transform;
const simplified = quantize(thinned, 1e5);
const simplePoints = countPoints(simplified);

// Every playable country must survive with a usable ring. A country simplified into
// nothing would render as a hole in the map and be unclickable as a polygon.
const simpleFeatures = feature(simplified, simplified.objects.countries);
const survivors = new Map();
for (const f of simpleFeatures.features) {
  if (f.id === undefined || f.id === null) continue;
  const id = String(f.id).padStart(3, '0');
  const rings = explode(f.geometry).filter((g) => g.coordinates[0]?.length >= 4);
  if (rings.length > 0) survivors.set(id, geoArea(f.geometry));
}
for (const c of countries) {
  if (!c.hasPolygon) continue;
  if (!survivors.has(c.id)) {
    errors.push(`${c.name}: simplification destroyed its geometry — raise RETAIN_POINTS`);
  }
}

fs.writeFileSync(path.join(OUT_DATA, 'world-50m.json'), JSON.stringify(simplified));
...
/*
 * Localised country names, emitted separately so the English roster above stays lean.
 *
 * world-countries carries a `translations` map per country, and every one of the 195 has
 * a common name in all of these languages — verified below, because a partial table
 * would silently fall back to English for a handful of countries and read as a bug.
 *
 * Capitals are NOT translated by the source and are therefore not emitted here. Capital
 * mode shows them in Latin script in every language; in a right-to-left interface that
 * needs explicit bidi isolation at the point of use, not a fix here.
 */
const NAME_LOCALES = { es: 'spa', fr: 'fra', de: 'deu', pt: 'por', ar: 'ara' };

const names = {};
for (const [locale, key] of Object.entries(NAME_LOCALES)) {
  const table = {};
  for (const c of countries) {
    const src = worldCountries.find((w) => w.cca3 === c.cca3);
    const translated = src?.translations?.[key]?.common;
    if (!translated) {
      errors.push(`${c.name}: no ${locale} (${key}) translation in world-countries`);
      continue;
    }
    table[c.id] = translated;
  }
  names[locale] = table;
}

fs.writeFileSync(
  path.join(OUT_SRC, 'countryNames.generated.ts'),
...
export const COUNTRY_NAMES: Readonly<Record<string, Readonly<Record<string, string>>>> = ${JSON.stringify(names, null, 2)};
`,
);
...
console.log(
  `  render geometry      ${simplePoints.toLocaleString()} points, down from ${rawPoints.toLocaleString()} ` +
    `(${(100 - (simplePoints / rawPoints) * 100).toFixed(0)}% fewer)`,
);
